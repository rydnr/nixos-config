...
export PYTHONEDA_ORG_ROOT_FOLDER=~/github/acmsl
export PATH=$PATH:~/.dry-wit/src:~/github/rydnr/tools/nix-dry-wit-scripts/update-sha256-nix-flake:~/github/rydnr/tools/nix-dry-wit-scripts/update-latest-inputs-nix-flake:~/github/rydnr/tools/nix-dry-wit-scripts/release-tag:~/github/pythoneda-def/pythoneda-shared-pythonlang/domain/sync-pythoneda:~/github/pythoneda-def/pythoneda-shared-pythonlang/domain/sync-pythoneda-project:~/.xmonad/bin
...
alias mcdstats='command memcached-tool 10.10.27.11:11211 stats'
alias mcdshow='command memcached-tool 10.10.27.11:11211 display'
...
# vim: syntax=sh ts=2 sw=2 sts=4 sr noet
  '';
        ...
        file.".bashrc-functions".text = ''
#function nix-env(){ nix-env -qa \* -P | fgrep -i "$1"; }

# find dirty repositories in pythoneda
...
'';
        ...
        file.".bashrc-privateexports".text = ''
...
'';
        ...
        file.".bashrc-privatealiases-bluetooth".text = ''
alias bt-phone="sudo hciconfig hci0 name \"(hostname)\"; echo 'connect 50:01:BB:19:61:74' | bluetoothctl";
...
'';
        ...
        file.".bashrc-privatefunctions".text = ''
...
# licdata functions
function licdata-find-client-by-id () {
  [ -z "$1" ] && echo "Usage: licdata-find-client-by-id clientId" && return 1;
  curl -v -X GET -H 'Content-Type: application/json' http://localhost:8800/api/clients/$1
}
function licdata-delete-client () {
  [ -z "$1" ] && echo "Usage: licdata-delete-client clientId" && return 1;
  curl -v -X DELETE -H 'Content-Type: application/json' http://localhost:8800/api/clients/$1
}
function licdata-query-clients () {
  query="$1"
  [ -n "$1" ] && query="?$1";
  curl -v -X GET -H 'Content-Type: application/json' http://localhost:8800/api/clients$query
}
function licdata-update-client () {
  id="$1"
  now=$(date '+%Y%m%d%H%M%s')
  [ -z "$id" ] && echo "Usage: licdata-update-client clientId" && return 1;
  curl -v -X POST -d "{ \"address\": \"$now-address\", \"contact\": \"$now@example.com\", \"phone\": \"+34$now\" }" -H "Content-Type: application/json" http://localhost:8800/api/clients/$id
}
# vim: syntax=sh ts=2 sw=2 sts=4 sr noet
'';
...
bindsym $mod+Shift+1 move workspace to output DP-2
bindsym $mod+Shift+2 move workspace to output DP-0
bindsym $mod+Shift+3 move workspace to output HDMI-0
...
bindsym $mod+exclam workspace $ws11
bindsym $mod+Shift+exclam move container to workspace $ws11
...
exec --no-startup-id firefox -P 4tv9d8tg.dev
exec --no-startup-id slack
exec --no-startup-id ~/.screenlayout/current.sh
'';

        file.".config/alacritty/alacritty.toml".text = ''
...
'';
  ...
  programs = {
    ...
    git = {
      ...
      userEmail = "jlo@colordigital.com";
    };
